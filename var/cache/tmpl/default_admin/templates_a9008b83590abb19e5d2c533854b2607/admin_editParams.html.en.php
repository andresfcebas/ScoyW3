    <?php if ($this->options['strict'] || (is_array($t->aParams)  || is_object($t->aParams))) foreach($t->aParams as $valueObj) {?>
        ...
        <?php } else {?>
            ...
            <?php }?>
            <?php if ($this->options['strict'] || (isset($t) && method_exists($t, 'isEqual'))) if ($t->isEqual($valueObj->type,"textarea")) { ?>
                <textarea name="<?php echo htmlspecialchars($valueObj->name);?>" id="<?php echo htmlspecialchars($valueObj->name);?>"><?php echo htmlspecialchars($valueObj->value);?></textarea>
            <?php }?>
